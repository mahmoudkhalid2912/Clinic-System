    ...
    : IRequestHandler<CreatePaymentIntentCommand, Result<PaymentIntentResponseDto>>
{
    ...
    {
        // 1. Find the booking
        var booking = await unitOfWork.BookinRepository.GetAsync(b => b.Id == request.BookingId, cancellationToken);
        if (booking is null)
            return Result.Failure<PaymentIntentResponseDto>(PaymentError.BookingNotFound);

        // 2. Find its payment record
        var payment = await unitOfWork.PaymentRepository.GetAsync(p => p.BookingId == request.BookingId, cancellationToken);
        if (payment is null)
